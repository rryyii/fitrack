		...
		register.addActionListener(e -> {
			SwingUtilities.invokeLater(new Runnable() {
				...
				public void run() {
					...
					userRegister.add(new JLabel("Height (Inches):"));
					userRegister.add(height);
					userRegister.add(new JLabel("Weight (Lbs):"));
					userRegister.add(weight);
					userRegister.add(new JLabel("Activity Level:"));
					userRegister.add(activity);

					int result = JOptionPane.showConfirmDialog(null, userRegister, "User Registration",
							JOptionPane.OK_CANCEL_OPTION);
					if (result == JOptionPane.CANCEL_OPTION) {
					} else if (connection.registerUser(username.getText(), password.getText(),
							Objects.requireNonNull(gender.getSelectedItem()).toString(),
							Integer.parseInt(weight.getText()), Integer.parseInt(height.getText()),
							Objects.requireNonNull(activity.getSelectedItem()).toString(),
