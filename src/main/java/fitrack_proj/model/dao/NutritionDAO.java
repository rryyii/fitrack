...
public class NutritionDAO {
	...
	public int insertNutrition(int userId, String foodName, Integer servingCount, int calories, int protein, int carbs,
			int fats, String mealType) {
		String insertNutrition = "INSERT INTO nutrition_history (user_id, date, food_name, serving_count, calories, protein, carbohydrates, fats, meal_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
		try {
			...
			insert.setInt(5, calories);
			insert.setInt(6, protein);
			insert.setInt(7, carbs);
			insert.setInt(8, fats);
			insert.setString(9, mealType);
			int result = insert.executeUpdate();
			return result;
		} catch (SQLException sqe) {
			System.out.println("Failed to insert a food log into the nutrition_history database: " + sqe.getMessage());
		}
		return -1;
	}
